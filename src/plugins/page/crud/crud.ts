...
export const CrudPlugin: FastifyPluginAsync = async (fastify, opts) => {
	fastify.addHook("preHandler", async (request, reply) => {
		const allModels = Object.keys(Array.from(fastify.database.entities).map((e) => e.getName()));
		allModels.sort((a, b) => a.localeCompare(b));
		reply.pageData.data["models"] = allModels; // serializer.serialize(users[0]);
	});
	fastify.get("/", async (request, reply) => {
		const models = Object.keys(Array.from(fastify.database.entities).map((e) => e.getName()));
		Object.assign(reply.pageData?.data, { models });
		return reply.renderTemplate();
	});

	fastify.get<{ Params: { model: string } }>("/:model", async (request, reply) => {
...
type SerializableAssociation = Omit<Association, "source" | "target" | "inspect"> & { through?: string };
...
function getAssociations(model: ModelCtor<any>) {
	...
	for (const assoc of Object.values(model.associations)) {
		...
		const joinTableName: string | undefined = (assoc as any)["throughModel"]?.name;
		if (joinTableName) {
